...
router.get('/xavier', async (req, res) => {
    const text = req.query.text;
    if (!text) {
        return res.status(400).json({ error: "Please provide text with the 'text' query parameter." });
    }

    const pathImg = path.resolve('./cache/xavier.png');

    try {
        // Fetch the base image
        const imageResponse = await axios.get('https://i.postimg.cc/NFXP7DbP/x9bnTJ9.jpg', { responseType: 'arraybuffer' });
...
const serviceMetadata = {
    name: "Xavier",
    author: "Jerome Jamis",
    description: "Creates an image with a comment in meme style",
    category: "Canvas",
    link: ["/api/xavier?text=<YOUR_TEXT>"]
};
